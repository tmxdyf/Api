...
class RequestExecutor {
    ...
    static {
        ...
        OkHttpClient client = new OkHttpClient.Builder().addInterceptor(chain -> {
            ...
            result = Api.isEncrypt() ? DESUtil.decryptDoNet(result) : result;//解密
...
//        初始化Retrofit
        Retrofit retrofit = new Retrofit.Builder().baseUrl(Api.getWebService())
                .client(client)
    ...
    public static <T> Observable<T> request(String method, Map<String, String> map, Class<T> cla) {
        Observable<String> observable = getApiService().post2(method, Api.isEncrypt() ? encryptMap(map) : map).subscribeOn(Schedulers.newThread()).observeOn(AndroidSchedulers.mainThread());
